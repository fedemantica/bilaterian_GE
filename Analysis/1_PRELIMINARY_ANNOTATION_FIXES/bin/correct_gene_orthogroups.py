...
double_genes = [gene for gene in resolved_chimeric_genes if gene in broken_genes] #list of genes that are both broken and chimeric, and have only been corrected as broken.
#remove the doube genes from the resolved chimeric list. In principle there is not need to add them to the unresolved. Since they have been fixed as broken, they should not be duplicated.
resolved_chimeric_genes = [gene for gene in resolved_chimeric_genes if gene not in double_genes] 
