...
use strict;
 ...
 ########################################################################################################################################################################################
 ########################################################################################################################################################################################
 #PROGRAM_NAME: get_exint_file.pl			
 #									  
 #PROGRAM DESCRIPTION: Program that gets a fasta protein file indicating aminoacid positions and phase of each exon in the corresponding protein sequence
 #
 #IMPORTANT REQUISITES TO RUN THE PROGRAM: 
 #
 #Two input files: 1) A GTF file with CDS information and 2) Genomic sequence in fasta format
 # 
 #BRIEF INTRUCTIONS FOR RUNNING THE PROGRAM:
 #
 # perl get_exint_file.pl -GTF <gtf_file> -G <genome.fa> -out <name_outfile>
 #
 # ########################################################################################################################################################################################
 ########################################################################################################################################################################################
...
while (<GTF>){
	...
	if ($line[2] eq "CDS"){
		...
		if (!$pid{$prot}){
			...
			#print "$gid\t$prot\t$line[0]\t$line[6]\tExon\t$line[3]\t$line[4]\t$phase\n";
