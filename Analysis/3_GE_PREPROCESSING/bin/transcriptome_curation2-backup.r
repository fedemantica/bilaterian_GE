...
check_within_tissue_correlation = function(tc, sra, dist_method, min_dif, selected_tissues) {
    ...
    for (sra_run in colnames(tc)) {
        ...
        if (my_tissue == "DigestiveTract") {next}
        if (grepl("_H$", sra_run)) {next} #Save the in-house samples.
        if (sra_run %in% c("Abdomen_Fatbody_a_a", "Abdomen_Fatbody_a_b", "Abdomen_Fatbody_a_c")) {next} #Save the Ame Adipose, that was being removed.
	if (sra_run %in% c("Brain_Fem_a", "Brain_Male_a")) {next} #Save the Mdo Neural, that was being removed.
	#if (sra_run %in% c("Brain_a_a1", "Brain_a_a2")) {exclude_runs = c(exclude_runs, sra_run); next} #try to exclude two zebrafish samples that I think are slightly problematic
...
print(length(rownames(tc_sva))) #Fede
print(length(rownames(sra))) #Fede
print(length(rownames(sva_out))) #Fede
#save_plot(tc_sva, sra, sva_out, dist_method, paste0(sub(' ', '_', scientific_name), '.', round,  '.original.sva'), selected_tissues, fontsize)

print("Round 0 done") #Fede
