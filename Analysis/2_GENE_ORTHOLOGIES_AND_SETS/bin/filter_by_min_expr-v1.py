...
not_annot_genes_list = list(set(list(orthogroups_df["GeneID"])) - set(list(all_species_geneID_max_expr_dict.keys())))
...
for OG_ID, group in orthogroups_grouped_df:
  ...
  for species in duplicated_species:
    ...
    genes_to_remove = [gene for gene in species_genes if all_species_geneID_max_expr_dict[gene] < float(min_expr)]
    if len(genes_to_remove) == len(species_genes):
      max_expr_species = max([all_species_geneID_max_expr_dict[key] for key in list(all_species_geneID_max_expr_dict.keys()) if key in species_genes])
      gene_to_keep = [gene for gene in genes_to_remove if all_species_geneID_max_expr_dict[gene] == max_expr_species][0]
      genes_to_remove = [gene for gene in genes_to_remove if gene != gene_to_keep] #update the list of genes to remove
      lowly_expr_genes_list.append(gene_to_keep)
    if len(genes_to_remove) > 0:
      removed_genes_list = removed_genes_list + genes_to_remove
      group = group.loc[~group["GeneID"].isin(genes_to_remove)]
