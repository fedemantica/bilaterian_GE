...
for gene_pair in filt_gene_pairs:
  ...
  ###### This is the only problematic gene, which I am excluding
  if geneID1=="EBAGB000039" or geneID2=="EBAGB000039":
    ...
    with open(output_sim_score_file, "w") as output_sim_score:
      output_sim_score.write("%s\t%s\t%s\t%s\t%s\t%f\n" % (my_orthogroup, species1, species2, geneID1, geneID2, np.nan))
      output_sim_score.write("%s\t%s\t%s\t%s\t%s\t%f\n" % (my_orthogroup, species1, species2, geneID1, geneID2, np.nan))
    continue
  #####
    ...
    if len(aln_output) == 0:
      ...
      if len(aln_output) == 0:
        ...
        if len(aln_output) == 0:
          ...
          if len(aln_output) == 0:
            time.sleep(60)
            aln_output = list(list(SeqIO.parse(output_aln_file, "fasta"))) #This is getting ridiculus
            if len(aln_output) == 0:
              time.sleep(500)
              aln_output = list(list(SeqIO.parse(output_aln_file, "fasta")))
  ...
  with open(output_sim_score_file, "w") as output_sim_score:
    ...
    output_sim_score.write("%s\t%s\t%s\t%s\t%s\t%f\n" % (my_orthogroup, species1, species2, geneID2, geneID1, sim_score[1]))
...
all_sim_scores_df = pd.read_table(all_sim_scores_file, sep="\t", index_col=False, header=None, names=["OG_ID", "Species1", "Species2", "GeneID1", "GeneID2", "Sim_score"])
