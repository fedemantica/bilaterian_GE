...
tissues = (Args[3])
species = (Args[4])
output_lengths_vertebrata_file = (Args[5])
output_lengths_insecta_file = (Args[6])
output_plot_file = (Args[7])
...
extract_branch_lengths = function(tissue_expr_table, collapsed_species, tissue, n_bootstraps) {
  ...
  if (exists("tissue_expr_table")) {
    ...
    #tissue_distance_matrix = cor(tissue_expr_table, method="spearman", use="complete.obs")
...
for (subclade in c("Vertebrata", "Insecta")) {
  ...
  if (subclade=="Insecta") {collapsed_species = c("Dme|Eba|Aae|BmA|Tca|Ame|Bge|Cdi")}
  ...
  if (subclade=="Vertebrata") {write.table(clade_values_df, output_lengths_vertebrata_file, sep="\t", quote=FALSE, row.names = FALSE)}
  if (subclade=="Insecta") {write.table(clade_values_df, output_lengths_insecta_file, sep="\t", quote=FALSE, row.names = FALSE)}
