...
<!-- Related supplementary panel: proportion of onholog orthogroups among the tissue-specific gains in all nodes -->
```{r onholog_proportion_plot_v1, warning=FALSE, message=FALSE, echo=FALSE, dependson=c("setup", "functions"), fig.width=9, fig.height=4}
####### Build color vector
deuterostome_colors = rep("olivedrab3", length(deuterostoma_y_labels)-1)
names(deuterostome_colors) = deuterostoma_y_labels[deuterostoma_y_labels!="Bilateria"]
protostome_colors = rep("tan3", length(protostoma_y_labels)-1)
...
tissue_gains_df$Onholog_status[tissue_gains_df$OG_ID %in% ohnolog_OGIDs] = "Ohn"
#Remove single species
#tissue_gains_df = subset(tissue_gains_df, !(TS_gains %in% ordered_species))
...
#                                    c(rep(rev(seq(1,9)),2)))
# #Add node level for plotting
# tissue_gains_df$Node_level = ancestor_node_level_dict$find(tissue_gains_df$TS_gains)
# tissue_gains_df$Node_level[tissue_gains_df$TS_gains=="Bilateria"] = 0

#tissue_gains_df$TS_gains = factor(tissue_gains_df$TS_gains, levels=all_ancestors)
# proportion_df = tissue_gains_df %>%
#   #filter(Tissue!="Testis") %>%
#   group_by(TS_gains, Onholog_status) %>%
#   summarise(n = n()) %>%
#   mutate(freq = n / sum(n)) %>%
#   mutate(freq1 = n / length(ohnolog_OGIDs)) %>%
#   filter(Onholog_status=="Ohn")
...
#proportion_df$Node_level = factor(proportion_df$Node_level, seq(0,9))
# test = ggplot(proportion_df, 
#                                  aes(x=Node_level, y=freq, 
#                                      group=Branch, color=Branch)) +
#   geom_point(size=4) +
#   geom_line(size=0.5) +
#   geom_text_repel(aes(label=TS_gains), box.padding = 0.5) +
#   #ggtitle("Total number of gains") +
#   scale_color_manual(values=c("Bilateria"="firebrick1", "Deuterostoma"="olivedrab4", "Protostoma"="goldenrod3")) +
#   theme_bw() +
#   theme(axis.text = element_text(color="black"),
...
#         #legend.justification = c("right", "top")
#         ) +
#   ylab("Propotion of Ohnolog orthogroups") +
#   scale_y_continuous(breaks = number_ticks(6)) +
#   xlab("Node level") +
#   guides(shape=FALSE)
# 
...
#Plot
test = ggplot(data=tissue_gains_df, aes(x=TS_gains, fill=Branch_Ohn_status, color=Vertebrate_status, alpha=Vertebrate_status)) +
