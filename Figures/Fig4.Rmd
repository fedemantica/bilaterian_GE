---
title: "Fig. 4: Tissue-specificity gains are associated with gene duplication and specialization."
...
<!-- FIRST ROW -->
<!-- Panel A: tree with total numbers of gains/losses across nodes and species -->
```{r tree_with_inerences_stats, warning=FALSE, echo=FALSE, message=FALSE, dependson=c("setup", "functions"), fig.width=11, fig.height=6, eval=FALSE}
...
for (tissue in ordered_tissues) {
  for (category in c("gains", "losses")) {
    ...
    #Isolate bilaterian "gains" in that tissue
    # if (category == "gains") {
    #   bilaterian_OGs = as.vector(subset(input_df, TS_gains=="Bilateria")$OG_ID)
    #   input_df = subset(input_df, !(OG_ID %in% bilaterian_OGs))
    # } else { #is the category is "losses", use the vector created in the previous iteration
    #   input_df = subset(input_df, !(OG_ID %in% bilaterian_OGs))
    # }
    #Count TS gains
    ...
    all_stats_df = rbind(all_stats_df, ts_gains_counts_df)
  }
}

#Sum gains/losses across tissues
all_collapsed_stats_df = all_stats_df %>%
  group_by(Node, Inference_type) %>%
  summarize(TS_stats = sum(TS_gains)) %>%
  mutate(y_coord = 1)
  

all_nodes_gains_losses_plots_list = list()
i = 1
ordered_nodes = unique(as.vector(all_collapsed_stats_df$Node))

for (my_node in ordered_nodes) {
  node_collapsed_stats_df = all_collapsed_stats_df %>% filter(Node == my_node)
  
  ####Generate plots with stats
  node_stats_plot = ggplotGrob(ggplot(data=node_collapsed_stats_df, 
                                      aes(x=1, y=y_coord)) +
                                 geom_point(shape=22, alpha = 0.7, color="black", aes(size=TS_stats, fill=Inference_type)) +
                                 
                                 geom_text(data=subset(node_collapsed_stats_df, Inference_type == "gains"), 
                                           aes(x=y_coord, y=y_coord, label=TS_stats, color=Inference_type), size=3, color="#443A83FF", hjust = 2, vjust = -0.7) +
                                 
                                 geom_text(data=subset(node_collapsed_stats_df, Inference_type == "losses"),
                                            aes(x=y_coord, y=y_coord, label=TS_stats, color=Inference_type), size=3, color="#75D054FF", hjust = 2, vjust = 0.7) +
                                 
                                 scale_fill_manual(values=c("gains"="#443A83FF", "losses"="#75D054FF")) +
                                 scale_color_manual(values=c("gains"="#443A83FF", "losses"="#75D054FF")) +
                                 scale_size_continuous(limits=c(0, max(all_collapsed_stats_df$TS_stats)),
                                                       range = c(1,15)) +
                                 theme_void() +
                                 #theme(panel.background = element_rect(fill = "white", color="white")) +
                                 guides(color=FALSE, size=FALSE, fill=FALSE))
  
  all_nodes_gains_losses_plots_list[[i]] = node_stats_plot
  i = i+1
}

#### 
node_gains_losses_stats_grob = tibble(xmin=node_x_position_dict$find(ordered_nodes)-0.75,
                                        ...
                                        grob=all_nodes_gains_losses_plots_list)

super_tree_test = plot_tree_structure_v1(node_x_position, node_y_position, node_x_labels) + #plot the tree structure
  purrr::pmap(node_gains_losses_stats_grob, function(grob, xmin, xmax, ymin, ymax) annotation_custom(grob = grob, xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax))

print(super_tree_test)
panel_A = super_tree_test

pdf("/Users/federica/Documents/CRG_PhD/bilaterian_GE/Science_submission/Figures_raw/Fig4_panelA-v2.pdf", width=11, height=6)
print(panel_A)
dev.off()
```
...
#save to file
pdf("/Users/federica/Documents/CRG_PhD/bilaterian_GE/Science_submission/Figures_raw/Fig4_panelB-v1.pdf", width=5, height=6)
print(panel_B)
dev.off()
...
#Do not plot Bilateria for now
gains_losses_nodes_plot = ggplot(subset(all_summed_gains_losses_df, Node!="Bilateria"), 
...
pdf("/Users/federica/Documents/CRG_PhD/bilaterian_GE/Science_submission/Figures_raw/Fig4_panelC-v1.pdf", width=6, height=6)
print(gains_losses_nodes_plot)
dev.off()
```
...
pdf("/Users/federica/Documents/CRG_PhD/bilaterian_GE/Science_submission/Figures_raw/Fig4_panelD-v1.pdf", width=7, height=4)
...
tissue_gains_df = vector()
for (tissue in ordered_tissues) {
  input_df = read.delim(paste0(ts_gains_losses_dir, my_version, "/inferences/Bilateria_conserved_orthogroups-All_genes-", tissue, "_inferred_gains.tab"), 
...
for (tissue in ordered_tissues) {
  ...
  ### Remove the relative bilaterian gains
  relative_gains_df = read.delim(paste0(ts_gains_losses_dir, my_version, "/inferences/Bilateria_conserved_orthogroups-All_genes-", tissue, "_inferred_gains.tab"), 
                                 header=TRUE, stringsAsFactors = FALSE, col.names = c("OG_ID", "TS_gains", "Category"))
  bilaterian_OGs = as.vector(subset(relative_gains_df, TS_gains=="Bilateria")$OG_ID)
  #### Remove from the lossess df
  input_df = subset(input_df, !(OG_ID %in% bilaterian_OGs))
  input_df$Tissue = rep(tissue, nrow(input_df))
...
pdf("/Users/federica/Documents/CRG_PhD/bilaterian_GE/Science_submission/Figures_raw/Fig4_panelE.pdf", width=12, height=7)
print(final_clade_expr_plot)
dev.off()
```
...
#### Save to pdf
pdf("/Users/federica/Documents/CRG_PhD/bilaterian_GE/Science_submission/Data_raw/figS9_panelD-v1.pdf", width=6, height=8)
...
write.table(ancestor_stacked_plots_df, "/Users/federica/Documents/CRG_PhD/bilaterian_GE/Science_submission/Data_raw/ancestor_stacked_plots_df.tab", quote=FALSE, row.names = FALSE, col.names=TRUE, sep="\t")
write.table(ancestor_stacked_plots_df, "/Users/federica/Documents/CRG_PhD/bilaterian_GE/Science_submission/Data_raw/ancestor_final_density_plots_df.tab", quote=FALSE, row.names = FALSE, col.names=TRUE, sep="\t")
```
...
pdf("/Users/federica/Documents/CRG_PhD/bilaterian_GE/Science_submission/Data_raw/figS9_panelE.pdf", width=6, height=8)
print(specialization_hyp_plot)
dev.off()

species_stacked_plots_df = all_stacked_plots_df
species_final_density_plots = final_density_plots_df
write.table(species_stacked_plots_df, "/Users/federica/Documents/CRG_PhD/bilaterian_GE/Science_submission/Data_raw/species_stacked_plots_df.tab", quote=FALSE, row.names = FALSE, col.names=TRUE, sep="\t")
write.table(species_stacked_plots_df, "/Users/federica/Documents/CRG_PhD/bilaterian_GE/Science_submission/Data_raw/species_final_density_plots_df.tab", quote=FALSE, row.names = FALSE, col.names=TRUE, sep="\t")
```
...
pdf("/Users/federica/Documents/CRG_PhD/bilaterian_GE/Science_submission/Figures_raw/Fig4_panelF.pdf", width=6, height=4)
print(combined_specialization_hyp_plot)
dev.off()
```
