...
<!-- Panel A: Alluvia for Multi-TS gains -->
```{r alluvia_multiple_ts_gains_different_tissues, echo=FALSE, message=FALSE, warning=FALSE, dependson=c("setup", "functions"), fig.height=6, fig.width=5, eval=FALSE}
...
## select the orthogroups with exclusively two gains
#OG_ID_gains_counts = table(tissue_gains_df$OG_ID)
#OG_ID_two_gains = names(OG_ID_gains_counts[OG_ID_gains_counts==2])
...
##Select the cases where the tissue is different for for Deuterostome and Protostome
...
two_gains_two_branches_two_tissues_df = as.data.frame(final_join_df)
two_gains_two_branches_two_tissues_df$Tissue = factor(two_gains_two_branches_two_tissues_df$Tissue, levels=ordered_tissues)
two_gains_two_branches_two_tissues_df$TS_gains = as.character(as.vector(two_gains_two_branches_two_tissues_df$TS_gains))

####### Some extra test

first_df = two_gains_two_branches_two_tissues_df


multiTS_alluvia_plot = ggplot() +
  geom_alluvium(data=first_df, aes(x=Clade, stratum=Tissue, alluvium=OG_ID, fill=Tissue), width=1/4) +
  stat_stratum(data=first_df, aes(x=Clade, stratum=Tissue, alluvium=OG_ID), width=1/4) +
  stat_stratum(data=first_df, aes(x=Clade, stratum=Tissue, alluvium=OG_ID, label=Tissue), geom = "text", size=3, width=1/4) +
  #Formatting and coloring
  scale_fill_manual(values=c(all_ancestors_colors, my_color_vector)) +
  theme_bw() +
  theme(axis.text = element_text(color="black"),
        plot.title = element_text(color="black", face="bold", hjust=0.5)) +
  scale_y_continuous(breaks = number_ticks(10)) +
  guides(fill=FALSE) +
  ggtitle(paste0("Different tissue specificity on the two branches: ", length(unique(two_gains_two_branches_two_tissues_df$OG_ID)), " OGs"))

pdf("/Users/federica/Documents/CRG_PhD/bilaterian_GE/Images/Images_from_R/new_Fig5/Fig5_panelL.pdf", width=8, height=8)
print(multiTS_alluvia_plot)
dev.off()

################# Save OG_IDs to file to compute GO enrichments
tissue_comb_df = first_df %>% 
  dplyr::select(-c(Clade, TS_gains)) %>% 
  group_by(OG_ID) %>% 
  mutate(tissue_comb  = paste0(Tissue, collapse="_")) %>%
  dplyr::select(-Tissue) %>%
  distinct()

#Save to file(s
all_tissue_combinations_df = as.data.frame(t(combn(ordered_tissues, 2)))
colnames(all_tissue_combinations_df) = c("Tissue1", "Tissue2")
all_tissue_combinations_df$Tissue1 = as.vector(all_tissue_combinations_df$Tissue1)
all_tissue_combinations_df$Tissue2 = as.vector(all_tissue_combinations_df$Tissue2)

for (index in seq(1, nrow(all_tissue_combinations_df))) {
  tissue_1 = all_tissue_combinations_df[index,"Tissue1"]
  tissue_2 = all_tissue_combinations_df[index,"Tissue2"]
  my_comb_df = tissue_comb_df %>%
    filter(grepl(tissue_1, tissue_comb)) %>%
    filter(grepl(tissue_2, tissue_comb)) %>%
    dplyr::select(-tissue_comb)
  #Write to corresponding file
  write.table(my_comb_df, paste0(ts_gains_losses_dir, my_version, "/multi_TS/", tissue_1, "_", tissue_2, "-OG_IDs-GO_input.txt"),
              col.names = FALSE, row.names = FALSE, quote=FALSE)
}
```

<!-- Panel B: upset plot with the number of exchanges between tissue compartments -->
```{r upset_plot_tissue_comb_number, echo=FALSE, message=FALSE, warning=FALSE, dependson=c("setup", "functions"), eval=FALSE, fig.height=8, fig.width=1.8}
tissue_gains_df = vector()
for (tissue in ordered_tissues) {
  input_df = read.delim(paste0(ts_gains_losses_dir, my_version, "/inferences/Bilateria_conserved_orthogroups-All_genes-", tissue, "_inferred_gains.tab"), header=TRUE, stringsAsFactors = FALSE, col.names = c("OG_ID", "TS_gains", "Category"))
  input_df$Tissue = rep(tissue, nrow(input_df))
...
####Subset to only OG_ID and Tissue
subsetted_df = two_gains_two_branches_two_tissues_df[,c("OG_ID", "Tissue")]
subsetted_ordered_df = subsetted_df[with(subsetted_df, order(OG_ID, Tissue)),]  #This is to have ordered tissues
subsetted_wide_df = aggregate(Tissue ~ OG_ID, subsetted_ordered_df, FUN=paste, collapse=";")

#Count combination
exchange_tissues_counts_df = as.data.frame(table(subsetted_wide_df$Tissue))
colnames(exchange_tissues_counts_df) = c("Tissue_comb", "Counts")
exchange_tissues_counts_ordered_df = exchange_tissues_counts_df[with(exchange_tissues_counts_df, order(Counts, decreasing = TRUE)),]
exchange_tissues_counts_ordered_df$Tissue_comb = factor(exchange_tissues_counts_ordered_df$Tissue_comb, levels=rev(exchange_tissues_counts_ordered_df$Tissue_comb))
exchange_tissues_counts_ordered_df$Levels = rep(2.2, nrow(exchange_tissues_counts_ordered_df))

my_tissue_combs = as.character(as.vector(exchange_tissues_counts_ordered_df$Tissue_comb))
exchange_tissues_plot_df = data.frame(Tissue_comb = rep(my_tissue_combs, 2),
                                      Tissue = c(sub(";.*", "", my_tissue_combs), sub(".*;", "", my_tissue_combs)),
                                      Levels = c(rep(1, length(my_tissue_combs)), rep(2, length(my_tissue_combs)))
                                      )

#Set level order for plotting
exchange_tissues_plot_df$Tissue_comb = factor(exchange_tissues_plot_df$Tissue_comb, levels = levels(exchange_tissues_counts_ordered_df$Tissue))
#Plot
multi_TS_exchange_plot = ggplot(data=exchange_tissues_plot_df, aes(x=Tissue_comb, y=Levels, group=Tissue_comb)) +
  geom_line(color="black", aes(fill=Tissue, color=Tissue)) +
  geom_point(size=3, aes(fill=Tissue, color=Tissue)) +
  geom_text(data=exchange_tissues_counts_ordered_df, aes(x=Tissue_comb, y=Levels, label=Counts)) +
...
pdf("/Users/federica/Documents/CRG_PhD/bilaterian_GE/Images/Images_from_R/new_Fig5/Fig5_panelM.pdf", width=1.8, height=8)
print(multi_TS_exchange_plot)
dev.off()
```

<!-- Panel C: GSEA enrichment -->
```{r developmental_GOs_gsea, warning=FALSE, echo=FALSE, message=FALSE, dependson=c("setup", "functions")}
...
#Definition of developmental categories
developmental_categories = as.vector(GO_species_specific_prop_df$GO_name)[grep("develop|differentiation|determination|morphogen|commitment|specification|regionalization|formation|genesis", as.vector(GO_species_specific_prop_df$GO_name))]
### developmental_GO_list: it has to be a list with index = [developmental] and value = list of develomental GOs
developmental_GO_list = list()
developmental_GO_list[["Developmental"]] = developmental_categories
### proportion_names_vec: it has to be a named vector with value = proportion of OGs with species-specific gains and name = category name
ordered_GOs_df = GO_species_specific_prop_df[order(as.vector(GO_species_specific_prop_df$species_specific_prop), decreasing = TRUE),]
proportion_names_vec = as.vector(ordered_GOs_df$species_specific_prop)
names(proportion_names_vec) = as.vector(ordered_GOs_df$GO_name)

fgseaRes <- fgsea(pathways=developmental_GO_list, stats=proportion_names_vec, minSize=0, maxSize=5000, nperm=100)

gsea_plot = plotEnrichment(developmental_GO_list[["Developmental"]], proportion_names_vec) + 
  labs(title="GSEA of developmental categories") +
  annotate("text", x = 5400, y=0.45, label = "p-value: 0.0099\nNES: 3.13\nSet size: 760\nTot categories: 5779") +
...
<!-- Panel D: Proportions of dev in the 95th percentile out of randomizations -->
```{r developmental_GOs_randomization_proportion, warning=FALSE, message=FALSE, echo=FALSE, dependson=c("setup", "functions")}
GO_transfers_df = read.delim(paste0(GO_transfers_dir, my_version, "/GO_annot/Hs2_transfers/orthogroups_from_Hs2-transferred_GO-reduced.txt"),
                             header=FALSE, col.names=c("OG_ID", "GO_term", "GO_name"))
...
#Add label regarding developmental categories
GO_species_specific_prop_df$Dev_label = rep("NO_Dev", nrow(GO_species_specific_prop_df))
GO_species_specific_prop_df$Dev_label[GO_species_specific_prop_df$GO_name %in% developmental_categories] = "Dev"

#Compute the proportion of categories in the 95% that are developmental.
...
perncetile_95_dev_proportion = nrow(subset(percentile_95_df, Dev_label=="Dev"))/nrow(percentile_95_df)

#Compute the same proportion over 100 randomizations of the developmental labels.
all_randomization_input_df = vector()
for (num in seq(1,100)) {
    randomization_df = GO_species_specific_prop_df
    randomization_df$rand_round = rep(num, nrow(randomization_df))
    all_randomization_input_df = rbind(all_randomization_input_df, randomization_df)
}

all_randomization_df = all_randomization_input_df %>%
  group_by(rand_round) %>%
  mutate(Dev_rand_label = sample(Dev_label)) %>%
  filter(species_specific_prop >= percentile_95 & Dev_rand_label=="Dev") %>%
  summarise(n = n()) %>%
  mutate(Dev_proportion = n/nrow(percentile_95_df))

randomization_dev_proportion_plot = ggplot(data=all_randomization_df, aes(x=Dev_proportion)) +
  geom_density(color="darkorchid3", fill="darkorchid3", alpha=0.7) +
  annotate("text", x=0.3, y=18, label = "Total randomizations: 100\nDashed line: observed proportion") +
  geom_vline(xintercept = perncetile_95_dev_proportion, color="black", linetype="dashed") +
  theme_bw() +
  xlab("Proportion of Developmental categories in the 95 percentile") +
  ylab("Density") +
  theme(axis.text = element_text(color="black"))
  
print(randomization_dev_proportion_plot)
```
