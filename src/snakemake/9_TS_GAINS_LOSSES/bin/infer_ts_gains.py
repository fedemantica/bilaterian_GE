...
node_name_dict = {"Hs2;Mm2":"Euarchontoglires", "Mm2":"Euarchontoglires", "Bt2":"Eutheria", "Mdo":"Mammals", "Gga":"Amniota", "Xtr":"Tetrapoda", "Dre":"Euteleostomi", "Cmi":"Vertebrata", "Bla":"Chordata", "Sp2":"Deuterostoma", "Dme;Eba":"Cyclorrapha", "Eba":"Cyclorrapha", "Aae":"Diptera", "BmA":"Panorpidae", "Tca":"Oligoneoptera", "Ame":"Holometabola", "Bge":"Neoptera", "Cdi":"Insecta", "Sma":"Arthropoda", "Obi":"Protostoma"}
  ...
  for OG_ID in OG_to_classify:
    ...
    if len(TS_species) > 0: #if there is at least one gene with 0.75
      if len(semi_TS_species) > 1: #and there is at least another gene with 0.60
        last_TS_species = semi_TS_species[-1] #isolate the last species with some tissue specificity
        node_name = node_name_dict[last_TS_species]
        result_dict[OG_ID] = node_name
      else: #if there is one TS gene and all the others are lower than 0.65
        result_dict[OG_ID] = TS_species[0]
      OG_to_classify = [element for element in OG_to_classify if element != OG_ID] #remove the OG_IDs from the ones to classify
...
#Header is: ["OG_ID", "Species", "Tau", "TS_labels"]
...
orthogroups_df = orthogroups_df.drop(columns=["TS_labels"])
