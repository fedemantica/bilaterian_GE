...
ancestral_clades = ["Chordata", "Deuterostoma", "Arthropoda", "Protostoma", "Vertebrata", "Insecta", "Euteleostomi", "Neoptera"]
...
for OG_ID in single_TS_OGs_deut:
  TS_species = list(orthogroups_df.loc[(orthogroups_df["OG_ID"]==OG_ID) & (orthogroups_df["Species"].isin(species_to_classify_deut)) & (orthogroups_df["Tau"] >= up_tau_cutoff)]["Species"])[0]
  OG_TS_classification_dict_deut[OG_ID] = TS_species

#Protostoma branch
for OG_ID in single_TS_OGs_prot:
  TS_species = list(orthogroups_df.loc[(orthogroups_df["OG_ID"]==OG_ID) & (orthogroups_df["Species"].isin(species_to_classify_prot)) & (orthogroups_df["Tau"] >= up_tau_cutoff)]["Species"])[0]
...
OG_to_classify_deut = [OG_ID for OG_ID in OG_to_classify if OG_ID not in no_TS_OGs and OG_ID not in single_TS_OG_deut]
print(len(OG_to_classify))
...
OG_to_classify_prot = [OG_ID for OG_ID in OG_to_classify if OG_ID not in no_TS_OGs and OG_ID not in single_TS_OG_prot]
...
ancestral_deut = [OG_ID for OG_ID in list(OG_TS_classification_dict_deut.keys()) if OG_TS_classification_dict_deut[OG_ID] in ancestral_clades] + [re.sub("\\..", "", OG_ID) for OG_ID in list(OG_TS_classification_dict_deut_LASSO.keys()) if OG_TS_classification_dict_deut_LASSO[OG_ID] in ancestral_clades]
ancestral_prot = [OG_ID for OG_ID in list(OG_TS_classification_dict_prot.keys()) if OG_TS_classification_dict_prot[OG_ID] in ancestral_clades] + [re.sub("\\..", "", OG_ID) for OG_ID in list(OG_TS_classification_dict_prot_LASSO.keys()) if OG_TS_classification_dict_prot_LASSO[OG_ID] in ancestral_clades]
...
for OG_ID in list(OG_TS_classification_dict_deut_LASSO.keys()):
  mod_ID = re.sub("\\..", "", OG_ID)
  if mod_ID in bilaterian_OG_IDs:
...
#remove multiple entries for the bilaterian classification
final_df = final_df.drop_duplicates(subset=["OG_ID", "TS_gain"])
#write to file
