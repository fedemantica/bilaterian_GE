...
for gene, group in grouped_df: #For each group
  ...
  if group.shape[0] == 2:
    ...
    if all([element != "NA" for element in list(group["first_aligned_ex_num"])]): #There are valid overlapping fragments for all the exons
      ...
      #If the intersection is of one element (exon)
        ...
        if len(ex_intersection) == 1:
          AA1_range = range(int(OG1["first_aligned_aa"]), int(OG1["first_aligned_aa"])+1)
          AA2_range = range(int(OG2["first_aligned_aa"]), int(OG2["first_aligned_aa"])+1)
    ...
    group_entry = "%s\t%s-%s\t%s-%s;%s-%s\t%s|%s;%s|%s\t%s\n" % (gene, OG1["orthogroup_id"], OG2["orthogroup_id"], OG1["first_aligned_ex_num"], OG1["last_aligned_ex_num"], OG2["first_aligned_ex_num"], OG2["last_aligned_ex_num"], OG1["first_aligned_ex"], OG1["last_aligned_ex"], OG2["first_aligned_ex"], OG2["last_aligned_ex"], chimeric_class)
